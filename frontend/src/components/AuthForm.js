...
function AuthForm({ isAuthenticated, setUser }) {
  const isLoginMode = isAuthenticated;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || err.response?.data?.error || 'An error occurred. Check if the backend is running.';
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <div className="auth-page">
      ...
      <div className="auth-card">
        <div className="auth-logo">
          ...
          <p>{isLoginMode ? 'Sign in to access your career tools' : 'Join AICareer Pro for free'}</p>
        </div>
